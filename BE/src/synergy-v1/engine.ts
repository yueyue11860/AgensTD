...
export function reconcilePlayerSynergies(input: {
  ...
  for (const [synergyId, previous] of previousById) {
    ...
    if (!next) deactivated.push(previous)
      ...
      || next.contributingGeneralIds.some((generalId, index) =>
        generalId !== previous.contributingGeneralIds[index])) {
    ...
    for (const activation of definition.levels) {
      ...
      for (const effect of activation.effects) {
        ...
        if (effectTarget.scope === 'owner_generals_with_facet'
          && !input.profiles.some((profile) => valuesForDimension(
            profile,
            effectTarget.dimension,
          ).includes(effectTarget.facetId))) {
