...
export const GameResources = memo(function GameResources({ resources }: GameResourcesProps) {
  return (
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      <ResourceMetric label="金币" value={resources.gold} icon={Coins} tone="text-yellow-400" />
      <ResourceMetric label="法力" value={resources.mana} max={resources.manaLimit} icon={Sparkles} tone="text-cold-blue" />
      <ResourceMetric label="热量" value={resources.heat} max={resources.heatLimit} icon={Flame} tone="text-warning-orange" />
      <ResourceMetric label="维修" value={resources.repair} icon={Wrench} tone="text-cyan-300" />
      <ResourceMetric label="主堡" value={resources.fortress} max={resources.fortressMax} icon={ShieldAlert} tone="text-alert-red" />
    </section>
  )
