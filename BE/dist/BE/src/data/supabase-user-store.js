...
exports.SupabaseUserStore = void 0;
...
class SupabaseUserStore {
    ...
    /** 查询用户进度，不存在则创建默认记录 */
    async getOrCreateProgress(playerId, playerType) {
        if (!this.client) {
    ...
    /** 记录关卡通关，更新进度 */
    async recordLevelClear(playerId, level, playerType, maxStandardLevel) {
    ...
    async getLevel5Leaderboard() {
        ...
        return data.map((row, index) => ({
            ...
            playerType: (row.player_type === 'AGENT' ? '硅基' : '碳基'),
            clearCount: row.level5_clear_count,
        }));
    }
}
exports.SupabaseUserStore = SupabaseUserStore;
