...
exports.STRUCTURED_ACTIVE_EFFECT_TYPES = exports.ALL_GENERAL_DEFINITIONS = exports.HOUYI_DEFINITION = void 0;
exports.collectNestedEffects = collectNestedEffects;
exports.collectSummonReferences = collectSummonReferences;
...
const curve = (one, two, three, four, five) => [one, two, three, four, five];
const EXPERIENCE_CURVE = [10000, 20000, 30000, 60000, 100000];
const BASE_STATS = {
    purple: {
        physical: {
...
const single = (priority = 'highest_current_hp') => ({
    kind: 'single', scope: 'enemies_in_attack_range', priority, targetLimit: 1,
});
const radius = (radiusMilliCells, targetLimit = 100) => ({
    kind: 'radius_aoe',
...
const line = (lengthMilliCells, halfWidthMilliCells = 500) => ({
    kind: 'line',
...
function damage(effectId, damageType, coefficientBpsByLevel, options = {}) {
    return {
        effectId,
        type: 'damage',
        damageType,
...
function status(effectId, statusId, magnitudeByLevel, durationMsByLevel, options = {}) {
    return {
        effectId,
        type: 'status_apply',
        statusId,
...
function passiveModifier(passiveId, stat, operation, value, target = { scope: 'self' }, condition) {
    return {
...
function defineGeneral(input) {
    const roster = (0, roster_1.getGeneralRosterEntry)(input.generalId);
    if (!roster)
        throw new Error(`Unknown roster general ${input.generalId}`);
    ...
    return {
        ...
        baseStats: {
            ...baseTemplate,
            attackRangeMilliCellsByLevel: fixed(input.attackRangeMilliCells ?? baseTemplate.attackRangeMilliCellsByLevel[0]),
        },
        basicAttack: {
            attackId: `${roster.generalId}_basic_attack`,
            targeting: { scope: 'enemies_in_radius', priority: 'furthest_progress', targetLimit: 1 },
            effect: damage(`${roster.generalId}_basic_attack_damage`, basicDamageType, fixed(10000), { criticalPolicy: 'can_crit', tags: ['direct', 'basic_attack'] }),
        },
        ...
        relatedSynergyIds: catalog_1.GENERAL_SYNERGY_IDS_BY_GENERAL[roster.generalId] ?? [],
    };
}
/** 后羿是已验收的纵向切片，下列数值必须保持原样。 */
exports.HOUYI_DEFINITION = {
    schemaVersion: 1,
    generalId: roster_1.GENERAL_IDS.HOUYI,
    name: '后羿',
    quality: 'purple',
...
const YANGJIAN = defineGeneral({
    generalId: roster_1.GENERAL_IDS.YANGJIAN,
...
const NAZHA = defineGeneral({
    generalId: roster_1.GENERAL_IDS.NAZHA,
...
const SHA_WUJING = defineGeneral({
    generalId: roster_1.GENERAL_IDS.SHA_WUJING,
    activeSkill: {
...
const ZHU_BAJIE = defineGeneral({
    generalId: roster_1.GENERAL_IDS.ZHU_BAJIE,
    attackRangeMilliCells: 2000,
    activeSkill: {
...
const YU_HUANG_DADI = defineGeneral({
    generalId: roster_1.GENERAL_IDS.YU_HUANG_DADI,
    activeSkill: {
...
const LEI_GONG = defineGeneral({
    generalId: roster_1.GENERAL_IDS.LEI_GONG,
    activeSkill: {
...
const DIAN_MU = defineGeneral({
    generalId: roster_1.GENERAL_IDS.DIAN_MU,
...
const ZHEN_YUANZI = defineGeneral({
    generalId: roster_1.GENERAL_IDS.ZHEN_YUANZI,
    activeSkill: {
...
const RU_LAI_FOZU = defineGeneral({
    generalId: roster_1.GENERAL_IDS.RU_LAI_FOZU,
    activeSkill: {
...
const PU_TI_LAOZU = defineGeneral({
    generalId: roster_1.GENERAL_IDS.PU_TI_LAOZU,
    activeSkill: {
...
const LIJING = defineGeneral({
    generalId: roster_1.GENERAL_IDS.LIJING,
    attackRangeMilliCells: 2000,
    activeSkill: {
...
const CHANG_E = defineGeneral({
    generalId: roster_1.GENERAL_IDS.CHANG_E,
...
const SUNWUKONG = defineGeneral({
    generalId: roster_1.GENERAL_IDS.SUNWUKONG,
    attackRangeMilliCells: 2000,
    activeSkill: {
...
const TAI_YI_ZHENREN = defineGeneral({
    generalId: roster_1.GENERAL_IDS.TAI_YI_ZHENREN,
    basicDamageType: 'magic',
    activeSkill: {
        ...
        effects: [{ effectId: 'tai_yi_zhenren_xiantong_summon', type: 'summon_unit', summonUnitId: summon_catalog_1.SUMMON_UNIT_IDS.LOTUS_IMMORTAL_CHILD, countByLevel: fixed(2), durationMsByLevel: fixed(20000), maxOwnedAliveByLevel: fixed(4), spawnRadiusMilliCellsByLevel: fixed(1000), spawnPattern: 'self_surrounding_empty_cells', inheritStatRatiosBps: { attack: 7000, attackSpeed: 10000, critRate: 10000, critDamage: 10000 }, sourceInactivePolicy: 'despawn', tags: ['active_skill', 'summon', 'burn', 'attack_speed_aura'] }],
    },
    passiveSkill: {
...
const SHOU_XING = defineGeneral({
    generalId: roster_1.GENERAL_IDS.SHOU_XING,
    activeSkill: {
...
const TANG_SANZANG = defineGeneral({
    generalId: roster_1.GENERAL_IDS.TANG_SANZANG,
    activeSkill: {
...
const BAI_LONGMA = defineGeneral({
    generalId: roster_1.GENERAL_IDS.BAI_LONGMA,
    activeSkill: {
...
const PI_LANPO = defineGeneral({
    generalId: roster_1.GENERAL_IDS.PI_LANPO,
    activeSkill: {
...
const GUAN_YIN_PUSA = defineGeneral({
    generalId: roster_1.GENERAL_IDS.GUAN_YIN_PUSA,
    activeSkill: {
...
const TAI_SHANG_LAOJUN = defineGeneral({
    generalId: roster_1.GENERAL_IDS.TAI_SHANG_LAOJUN,
    activeSkill: {
...
exports.ALL_GENERAL_DEFINITIONS = [
    YANGJIAN,
    NAZHA,
    exports.HOUYI_DEFINITION,
    SHA_WUJING,
...
exports.STRUCTURED_ACTIVE_EFFECT_TYPES = new Set(exports.ALL_GENERAL_DEFINITIONS.flatMap((definition) => definition.activeSkill.effects.map((effect) => effect.type)));
function collectNestedEffects(effects) {
    const output = [];
    for (const effect of effects) {
        output.push(effect);
        if (effect.type === 'spawn_zone')
            output.push(...collectNestedEffects(effect.tickEffects));
    }
    return output;
}
function collectSummonReferences(definition) {
