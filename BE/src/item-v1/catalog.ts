import {
  ...
  StructuredItemEffect,
} from './types'

const ui = (shortDescription: string, detailDescription: string, iconKey: string) => ({
  shortDescription,
  detailDescription,
  iconKey,
})

export const ACTIVE_ITEM_DEFINITIONS = [
  {
    schemaVersion: 1,
...
] as const satisfies readonly ActiveItemDefinition[]

function timedGeneralEffect(
  effectId: string,
...
const passiveBase = (
  ...
  itemId,
  name,
  itemKind: 'passive' as const,
  tags,
  ui: ui(shortDescription, shortDescription, `item_${itemId}`),
  status: 'released' as const,
  scope: 'owner_global' as const,
  attachAt: 'player_match_state_initialized' as const,
  effects: [] as StructuredItemEffect[],
  ruleModifiers: [],
  eventListeners: [],
  stacking: { group: `passive_item:${itemId}`, policy: 'unique' as const },
})

export const PASSIVE_ITEM_DEFINITIONS = [
  {
...
] as const satisfies readonly PassiveItemDefinition[]

function persistentEffect(
  effectId: string,
  target: Extract<StructuredItemEffect, { type: 'persistent_stat_modifier' }>['target'],
  stat: Extract<StructuredItemEffect, { type: 'persistent_stat_modifier' }>['stat'],
  valueBps: number,
): StructuredItemEffect {
  return {
    effectId,
...
export const ACTIVE_ITEM_IDS = ACTIVE_ITEM_DEFINITIONS.map((definition) => definition.itemId)
export const PASSIVE_ITEM_IDS = PASSIVE_ITEM_DEFINITIONS.map((definition) => definition.itemId)

const ITEM_BY_ID = new Map<string, ItemDefinition>(
  ITEM_DEFINITIONS.map((definition) => [definition.itemId, definition]),
)
...
export function getActiveItemDefinition(itemId: string): ActiveItemDefinition | undefined {
  const definition = getItemDefinition(itemId)
  return definition?.itemKind === 'active' ? definition : undefined
}

export function getPassiveItemDefinition(itemId: string): PassiveItemDefinition | undefined {
  const definition = getItemDefinition(itemId)
  return definition?.itemKind === 'passive' ? definition : undefined
}

export function validateItemCatalog(definitions: readonly ItemDefinition[] = ITEM_DEFINITIONS): readonly string[] {
  const errors: string[] = []
  ...
  for (const definition of definitions) {
    if (definition.schemaVersion !== 1) errors.push(`${definition.itemId}: invalid schemaVersion`)
    if (!/^[a-z][a-z0-9_]*$/.test(definition.itemId)) errors.push(`${definition.itemId}: invalid itemId`)
    if (itemIds.has(definition.itemId)) errors.push(`${definition.itemId}: duplicate itemId`)
    ...
    if (definition.itemKind === 'active') {
      ...
      if (definition.availabilityPhases.length === 0) errors.push(`${definition.itemId}: no phases`)
      if (definition.effects.length + definition.actions.length === 0) errors.push(`${definition.itemId}: no executable payload`)
      if (definition.failurePolicy !== 'no_consume') errors.push(`${definition.itemId}: invalid failure policy`)
    } else {
      if (definition.scope !== 'owner_global') errors.push(`${definition.itemId}: V1 items must be owner_global`)
      if (definition.attachAt !== 'player_match_state_initialized') errors.push(`${definition.itemId}: invalid attach point`)
      if (definition.effects.length + definition.ruleModifiers.length === 0) errors.push(`${definition.itemId}: no passive payload`)
    }
    ...
    for (const id of ids) {
      if (!id.startsWith(`${definition.itemId}.`)) errors.push(`${definition.itemId}: nested id ${id} lacks item prefix`)
      if (nestedIds.has(id)) errors.push(`${definition.itemId}: duplicate nested id ${id}`)
      nestedIds.add(id)
    }
  }
  ...
  for (const id of ACTIVE_ITEM_IDS) if (!expectedActive.delete(id)) errors.push(`unexpected/duplicate active item ${id}`)
  for (const id of PASSIVE_ITEM_IDS) if (!expectedPassive.delete(id)) errors.push(`unexpected/duplicate passive item ${id}`)
  for (const id of expectedActive) errors.push(`missing active item ${id}`)
  for (const id of expectedPassive) errors.push(`missing passive item ${id}`)

  return errors
}

export function assertValidItemCatalog(): void {
