...
export type SummonUnitId = typeof SUMMON_UNIT_IDS[keyof typeof SUMMON_UNIT_IDS]

export interface SummonUnitTemplate {
  schemaVersion: 1
  ...
  baseStats: {
    attackByOwnerLevel: LevelCurve
    attackIntervalMsByOwnerLevel: LevelCurve
    attackRangeMilliCellsByOwnerLevel: LevelCurve
    critChanceBpsByOwnerLevel: LevelCurve
    critDamageBpsByOwnerLevel: LevelCurve
  }
  basicAttack: {
    attackId: string
  ...
  onHitDamageOverTime: null | {
    effectId: string
    damageType: 'physical' | 'magic'
    ownerAttackCoefficientBpsPerTick: number
...
export const SUMMON_UNIT_CATALOG = {
  [SUMMON_UNIT_IDS.CELESTIAL_SOLDIER]: {
    schemaVersion: 1,
    summonUnitId: SUMMON_UNIT_IDS.CELESTIAL_SOLDIER,
    displayName: '天兵',
    glyph: '兵',
    damageType: 'physical',
  ...
  [SUMMON_UNIT_IDS.MOON_RABBIT]: {
    schemaVersion: 1,
    summonUnitId: SUMMON_UNIT_IDS.MOON_RABBIT,
    displayName: '月兔',
    glyph: '兔',
    damageType: 'magic',
  ...
  [SUMMON_UNIT_IDS.MONKEY_SOLDIER]: {
    schemaVersion: 1,
    summonUnitId: SUMMON_UNIT_IDS.MONKEY_SOLDIER,
    displayName: '猴兵',
    glyph: '猴',
    damageType: 'physical',
  ...
  [SUMMON_UNIT_IDS.LOTUS_IMMORTAL_CHILD]: {
    schemaVersion: 1,
    summonUnitId: SUMMON_UNIT_IDS.LOTUS_IMMORTAL_CHILD,
    displayName: '莲花仙童',
    glyph: '童',
    damageType: 'magic',
...
} as const satisfies Readonly<Record<SummonUnitId, SummonUnitTemplate>>

export function validateSummonUnitCatalog(
  catalog: Readonly<Record<string, SummonUnitTemplate>> = SUMMON_UNIT_CATALOG,
): void {
