...
function createAgentApiRouter(engine, config, replayRecorder, competitionStore) {
    ...
    router.get('/stream', (request, response) => {
        ...
        response.write(`data: ${JSON.stringify((0, state_projection_1.projectFrontendGameState)(engine.getStateSnapshot(), config))}\n\n`);
        const unsubscribe = engine.onTick((state) => {
            response.write(`event: tick_update\n`);
            response.write(`data: ${JSON.stringify((0, state_projection_1.projectFrontendGameState)(state, config))}\n\n`);
        });
