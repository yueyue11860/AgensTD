import type { CombatEventBatch, GameState, GameStatePatch, PveEnemyState } from '../../../shared/contracts/game'

export const E2E_RENDERER_STRESS_ROOM = 'E2E-RENDERER-STRESS'
export const E2E_RENDERER_STRESS_MATCH = 'e2e-renderer-protocol-stress'
...
const boardPieces = Array.from({ length: 10 }, (_, index) => ({
  entityId: `stress-soldier-${index}`, ownerPlayerId: 'human-dev', kind: 'soldier' as const,
  glyph: ['刀', '枪', '弓', '骑'][index % 4], soldierType: ['blade', 'spear', 'bow', 'cavalry'][index % 4] as 'blade' | 'spear' | 'bow' | 'cavalry',
  level: 3 as const, nextAttackTick: 0, x: 10 + index % 5, y: 17 + Math.floor(index / 5) * 2,
}))

export function stressEnemies(tick: number): PveEnemyState[] {
  const glyphs = ['鬼', '怪', '妖', '魅']
  return Array.from({ length: 80 }, (_, index) => {
    const progress = (index * 270 + tick * (8 + index % 5)) % 24_000
    const side = Math.floor(progress / 6_000); const offset = (progress % 6_000) / 1000
...
export function stressFullState(tick = 0): GameState {
  const enemies = stressEnemies(tick)
  return {
    matchId: E2E_RENDERER_STRESS_MATCH, tick, status: 'running', result: null,
...
export function stressPatch(tick: number): GameStatePatch {
  const enemies = stressEnemies(tick)
  return {
...
export function stressCombatBatch(sequence: number, tick: number): CombatEventBatch {
  const target = `stress-enemy-${sequence % 80}`
  return {
    matchId: E2E_RENDERER_STRESS_MATCH, presentationVersion: 1, fromSeq: sequence, toSeq: sequence,
