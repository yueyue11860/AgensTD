...
class ProgressStore {
     ...
     */
    getOrCreate(playerId, playerType) {
     ...
     */
    recordLevelClear(playerId, level, playerType) {
        const progress = this.getOrCreate(playerId, playerType);
        // 下一关解锁（上限为隐藏关前的最后一关，隐藏关由 unlock-logic 单独控制）
        const nextLevel = level + 1;
        if (nextLevel > progress.highestUnlockedLevel && level <= progress_1.MAX_STANDARD_LEVEL) {
