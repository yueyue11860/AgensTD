...
const engine = new game_engine_1.GameEngine(config);
...
const replayRecorder = new replay_recorder_1.ReplayRecorder(engine, config, competitionStore);
const actionLimiter = new action_rate_limiter_1.ActionRateLimiter(config.actionRateLimitWindowMs, config.actionRateLimitMax);
const gateway = new socket_gateway_1.SocketGateway(httpServer, engine, config, actionLimiter);
app.use('/api', (0, rest_api_1.createRestApiRouter)(engine, config, actionLimiter, replayRecorder, competitionStore));
app.use('/api/agent', (0, agent_api_1.createAgentApiRouter)(engine, config, replayRecorder, competitionStore));
