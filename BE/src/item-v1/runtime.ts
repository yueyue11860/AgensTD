import { getActiveItemDefinition } from './catalog'
import {
  ActiveItemRuntimeState,
  ActiveItemTarget,
  ItemErrorCode,
  ItemExecutionPlan,
  ItemRuntimeAggregate,
  ItemRuntimeContext,
  ItemUseReceipt,
  MatchItemLoadoutSnapshot,
  UseActiveItemCommand,
  UseActiveItemResult,
} from './types'

export function createItemRuntimeAggregate(
  matchId: string,
  snapshot: MatchItemLoadoutSnapshot,
): ItemRuntimeAggregate {
  const slots = snapshot.activeSlots.map((itemId, index): ActiveItemRuntimeState | null => {
    if (!itemId) return null
    const definition = snapshot.activeItems.find((candidate) => candidate.itemId === itemId)
    if (!definition) throw new Error(`Snapshot is missing equipped active item ${itemId}`)
    return {
      itemId,
  ...
  }) as [ActiveItemRuntimeState | null, ActiveItemRuntimeState | null]
...
export function useActiveItem(
  ...
  command: UseActiveItemCommand,
  context: ItemRuntimeContext,
): UseActiveItemResult {
  const previous = state.processedRequests[command.requestId]
  if (previous) return receiptToResult(previous, command.requestId, state)

  const reject = (error: ItemErrorCode): UseActiveItemResult => {
    const receipt: ItemUseReceipt = { ok: false, runtimeVersion: state.version, error }
    const next = {
      ...state,
  ...
  if (!command.requestId || command.playerId !== state.playerId) return reject('ITEM_NOT_EQUIPPED')
  if (command.expectedItemRuntimeVersion !== state.version) return reject('STALE_ITEM_RUNTIME_VERSION')
  const slot = state.slots[command.slotIndex]
  if (!slot || slot.itemId !== command.itemId) return reject('ITEM_NOT_EQUIPPED')
  const definition = getActiveItemDefinition(command.itemId)
  if (!definition) return reject('ITEM_NOT_FOUND')
  if (!slot.enabled || slot.chargesRemaining <= 0) return reject('NO_ITEM_CHARGES')
  if (context.currentTick < slot.cooldownEndsAtTick) return reject('ITEM_ON_COOLDOWN')
  ...
  if (!targetShapeMatches(definition.targeting.kind, command.target)) return reject('INVALID_ITEM_TARGET')

  const validation = context.validateTarget(definition, command.target)
  if (!validation.ok) return reject(validation.error ?? 'INVALID_ITEM_TARGET')
  if (validation.hasLegalTarget === false) return reject('INVALID_ITEM_TARGET')

  const useSequence = state.nextUseSequence
  const plan: ItemExecutionPlan = {
  ...
  const nextSlot: ActiveItemRuntimeState = {
    ...slot,
  ...
  const nextSlots = [...state.slots] as [ActiveItemRuntimeState | null, ActiveItemRuntimeState | null]
  nextSlots[command.slotIndex] = nextSlot
  const nextVersion = state.version + 1
  const receipt: ItemUseReceipt = { ok: true, runtimeVersion: nextVersion, plan }
  const next: ItemRuntimeAggregate = {
    ...state,
    version: nextVersion,
...
function targetShapeMatches(
  expected: 'none' | 'character_token' | 'active_general' | 'battlefield_point' | 'discarded_character_to_empty_slot',
