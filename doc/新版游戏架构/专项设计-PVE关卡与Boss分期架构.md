...
> **历史文档覆盖声明（2026-08-25）**：Boss 的正式内容、数值、运行时、奖励和验收以《专项设计-BOSS系统完整落地.md》为权威。本文件第 1.2、3.2、11.1 中“私有段只能路线主人攻击、进入公共圈后才可协防”的规则已经废弃。新版 PVE 棋盘全局可部署，任何玩家单位只要满足几何射程和目标规则，即可攻击任一路线的小怪或 Boss；路线归属只用于出生、清波和奖励记账，不限制部署或攻击。

> 补充权威文档：三难度、30 个关卡变体解锁、当前 20 波数值与碎片掉落以《专项设计-PVE三难度解锁、数值与武器掉落.md》为准。本文保留场景、路线、波次状态机和 Boss 插件边界。

> 状态：历史分期方案；Boss 已完成全量纵向落地
> 当前实现：10 关、每关 W5/W10/W15/W20 共 40 个 Boss 节点，三难度合计 120 个解析变体；正式规则与验收以《专项设计-BOSS系统完整落地.md》为准
> 权威边界：本文件只定义关卡编排、分路波次、Boss 插件和阶段完成；天兵/普通小怪数值、战斗公式、掉落内容与局外结算由对应专项负责

## 1. 结论与不可改变的边界

1. 继续复用原项目 29×29 战场、P1～P4 槽位、四条出生路线和公共循环圈，不重做战场拓扑。
2. 每位玩家拥有一条出生与清波记账路线，但棋盘全局可部署；任何玩家单位只要满足几何射程和目标规则，就能攻击任一路线的敌人。
...
6. 当前版本已经完成 20 波和 W5/W10/W15/W20 四个正式 Boss 节点；后续新增 Boss 只能扩展目录、版本化插件与白名单效果命令。
7. 击杀收益归最后有效伤害玩家；出生路线不预绑定斋饭或经验。路线主人另按独立清波条件获得 `5 × 波次` 斋饭保底。
8. 每波每条有效路线固定生成 10 只普通小怪；关卡只能改变字池、场景和 Boss 组合，不能改变普通小怪数量。
9. 单关最多引用 4 种普通小怪，20 波内逐步引入并在后期混合。
...
- 普通小怪组 `countPerLane !== 10`，或关卡普通小怪定义超过 4 种。
- 负延迟、非正整数间隔或非整数 Tick。
...
- `prep`：等待本波准备时间。首波在玩家选定关卡并开始对局后完整等待 5 秒，期间保持零怪；后续波次也固定等待 5 秒。
- `spawning`：各有效路线按相同脚本、独立计数生成。
- `clearing`：所有仍有效生成组均已结束，但至少一条路线仍有本波存活实体。
- `completed`：所有未永久退出的有效路线均完成；持久化检查点后进入下一波准备阶段。

第一版允许场上同时存在不同波次的小怪：当前波完成全部出怪后立即开始下一波的 5 秒准备倒计时，不等待旧怪清空。所有实体必须保存 `waveNumber` 与出生路线，清波奖励仍按各路线、各波的存活实体独立结算，避免奖励提前发放。
...
本段是历史接口草案。`statsRef`、控制抗性、Boss 生命/防御与击杀奖励现已由《专项设计-BOSS系统完整落地.md》和 `boss-catalog.ts` 权威定义，不再以本文件的占位字段为准。
