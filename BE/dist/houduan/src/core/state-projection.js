...
function projectFrontendGameState(state, config) {
    ...
    return {
        ...
        towers: state.towers.map((tower) => ({
            id: tower.id,
            type: tower.type,
            name: tower_catalog_1.towerCatalog[tower.type]?.label ?? tower.type,
            level: 1,
        ...
        buildPalette: Object.values(tower_catalog_1.towerCatalog).map((entry, index) => ({
            type: entry.type,
            label: entry.label,
