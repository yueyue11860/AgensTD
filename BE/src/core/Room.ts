...
export class Room {
  ...
  private readonly pendingGeneralSelections = new Map<string, readonly string[]>()
  ...
  setPlayerGeneralSelection(playerId: string, selectedGeneralIds: readonly string[]): boolean {
    if (!this.getConnectedPlayerIds().includes(playerId) || this.getPhase() === 'playing') return false
    if (!Array.isArray(selectedGeneralIds)
      || selectedGeneralIds.length === 0
      || selectedGeneralIds.length > MAX_GENERAL_SELECTIONS_PER_MATCH
      || selectedGeneralIds.some((id) => typeof id !== 'string')) return false
    ...
    if (locked?.unlockedGeneralIds
      && normalized.some((generalId) => !locked.unlockedGeneralIds!.includes(generalId))) return false
    ...
    if (locked && this.phase !== 'playing') {
      ...
      this.engine.setMatchBuildSnapshots(Object.fromEntries(
        [...this.matchBuildSnapshots.entries()].map(([id, snapshot]) => [id, structuredClone(snapshot)]),
      ))
    }
    return true
  }
  ...
  restorePveCheckpointPayload(raw: Record<string, unknown>): void {
    const checkpoint = structuredClone(raw) as {
      ...
      pendingGeneralSelections?: Array<[string, string[]]>
    ...
    for (const [playerId, ids] of checkpoint.pendingGeneralSelections ?? []) this.pendingGeneralSelections.set(playerId, [...ids])
