...
export function buildEncyclopediaCatalog(account: Readonly<PlayerAccountRecord>): EncyclopediaCatalogPayload {
  ...
  const encounteredBosses = new Set<string>()
  const addEncounter = (
    stageSelection: { levelId: number } | undefined,
    highestCompletedWave: number,
    highestEncounteredWave: number | undefined,
    reason: string,
  ) => {
    const wave = highestEncounteredWave
      ?? (reason === 'victory' ? 20 : Math.min(20, highestCompletedWave + 1))
    if (!Number.isSafeInteger(wave) || wave < 0) return
    for (let number = 1; number <= Math.min(20, wave); number += 1) encounteredWaves.add(number)
  ...
  Object.values(account.settlementsById).forEach((settlement) => {
    addEncounter(
      ...
      settlement.highestCompletedWave,
      settlement.highestEncounteredWave,
      settlement.reason,
    )
  })
