...
exports.InMemoryPvpMatchmakingService = void 0;
...
class InMemoryPvpMatchmakingService {
    clock;
    ids;
    tickets = new Map();
    activeTicketByPlayer = new Map();
    proposals = new Map();
    acceptedMatches = new Map();
    requestReceipts = new Map();
    confirmFailureCount = new Map();
    cooldownUntilByPlayer = new Map();
    proposalAssignmentSequence = 0;
    constructor(clock = new SystemClock(), ids = new SequentialIdFactory()) {
        this.clock = clock;
        this.ids = ids;
    }
    join(command) {
        ...
        return this.idempotent(principal.playerId, request.requestId, 'queue_join', { request, profile }, () => {
            if (principal.kind !== 'human')
                return { ok: false, code: 'HUMAN_ACCOUNT_REQUIRED' };
            if (!principal.playerId.trim() || !principal.playerName.trim())
                return { ok: false, code: 'INVALID_PRINCIPAL' };
            if (!request.requestId.trim())
                return { ok: false, code: 'REQUEST_ID_REQUIRED' };
            if (request.mode !== 'ranked_1v1' && request.mode !== 'casual_1v1')
                return { ok: false, code: 'UNSUPPORTED_MODE' };
            if (!request.region.trim() || !request.rulesetVersion.trim())
                return { ok: false, code: 'INVALID_QUEUE_DIMENSIONS' };
            if (!Number.isSafeInteger(request.loadoutVersion) || request.loadoutVersion < 0)
                return { ok: false, code: 'INVALID_LOADOUT_VERSION' };
            if (!Number.isFinite(profile.rating) || profile.rating < 0)
                return { ok: false, code: 'INVALID_RATING_SNAPSHOT' };
    ...
    cancel(playerId, ticketId, requestId) {
        return this.idempotent(playerId, requestId, 'queue_cancel', { ticketId }, () => {
            ...
            if (ticket.state === 'accepted')
                return { ok: false, code: 'MATCH_ALREADY_ACCEPTED' };
            ...
            if (ticket.proposalId)
                this.expireProposal(ticket.proposalId, playerId, true);
    ...
    accept(playerId, proposalId, requestId) {
        return this.idempotent(playerId, requestId, 'match_accept', { proposalId }, () => {
            ...
            if (!player)
                return { ok: false, code: 'PLAYER_NOT_IN_PROPOSAL' };
    ...
    advance() {
        ...
        for (const proposal of [...this.proposals.values()]) {
            if (now >= proposal.found.confirmDeadlineAt)
                this.expireProposal(proposal.found.proposalId);
        }
        this.tryCreateProposals();
    ...
    canMatch(left, right) {
        if (left.playerId === right.playerId)
            return false;
        if (left.mode !== right.mode || left.rulesetVersion !== right.rulesetVersion || left.region !== right.region)
            return false;
        if ((left.isPlacement && right.leaderboardRank !== null && right.leaderboardRank <= 200)
            || (right.isPlacement && left.leaderboardRank !== null && left.leaderboardRank <= 200))
            return false;
    ...
    candidatePriority(anchor, left, right) {
    ...
    expireProposal(proposalId, cancelledPlayerId, explicitCancel = false) {
        const proposal = this.proposals.get(proposalId);
        if (!proposal)
            return;
        this.proposals.delete(proposalId);
        ...
        for (const proposalPlayer of proposal.found.players) {
            ...
            if (!cancelledSelf)
                this.applyConfirmationFailure(ticket.playerId, now);
        }
    }
    applyConfirmationFailure(playerId, now) {
        ...
        if (cooldownMs > 0)
            this.cooldownUntilByPlayer.set(playerId, now + cooldownMs);
    }
    publicTicket(ticket) {
...
exports.InMemoryPvpMatchmakingService = InMemoryPvpMatchmakingService;
