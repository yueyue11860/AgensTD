...
exports.createSettlementTelemetry = createSettlementTelemetry;
exports.ingestSettlementEvents = ingestSettlementEvents;
exports.resolveSettlementStats = resolveSettlementStats;
exports.scorePveSettlementMvp = scorePveSettlementMvp;
exports.buildPveSettlementDetail = buildPveSettlementDetail;
exports.finalizePveSettlementDetail = finalizePveSettlementDetail;
const emptyStats = () => ({
    damageDealt: 0,
    kills: 0,
    controlAppliedMs: 0,
...
function createSettlementTelemetry(matchId) {
...
function playerStats(telemetry, playerId) {
...
function ingestSettlementEvents(telemetry, events) {
    for (const event of events) {
        if (telemetry.seenEventIds.has(event.id))
            continue;
        telemetry.seenEventIds.add(event.id);
        if (event.type === 'MATCH_STARTED')
            telemetry.sawMatchStarted = true;
        ...
        if (event.type === 'STATUS_APPLIED' && typeof event.data.generalId === 'string'
            ...
            telemetry.controlByGeneralId.set(event.data.generalId, (telemetry.controlByGeneralId.get(event.data.generalId) ?? 0) + duration);
        }
    }
}
function resolveSettlementStats(telemetry, players) {
    ...
    const ownerByGeneralId = new Map();
    for (const player of players)
        for (const general of player.generalProgress) {
    ...
    for (const [generalId, duration] of telemetry.controlByGeneralId) {
        ...
        if (owner)
            resolved.get(owner).controlAppliedMs += duration;
    }
    return resolved;
}
function metricContribution(value, total, weightBps) {
    return total > 0 ? Math.round(value / total * weightBps) : 0;
}
function scorePveSettlementMvp(byPlayerId, coverageComplete) {
    if (!coverageComplete || byPlayerId.size < 2)
        return null;
...
function failureSuggestion(reason, wave) {
    if (reason === 'victory')
        return null;
    if (reason === 'disconnect_exit')
        return '检查网络后重连；宽限期内回房可继续本局，避免提前结算。';
    if (reason === 'voluntary_exit')
        return '再战前先调整构筑，确保主力神将与至少一组羁绊成型。';
    if (wave < 5)
        return '前五波先优先合成一名主力神将，不要将饭全部分散到低星棋子。';
    if (wave < 10)
        return '下次在 Boss 波前留出道具与技能冷却，并补一名控制或破甲神将。';
...
function buildPveSettlementDetail(input) {
    const stats = input.allStats.get(input.player.playerId) ?? emptyStats();
    const coreProgress = [...input.player.generalProgress].sort((left, right) => (stats.damageByGeneralId[right.generalId] ?? 0) - (stats.damageByGeneralId[left.generalId] ?? 0)
        || right.level - left.level || left.generalId.localeCompare(right.generalId))[0];
    const rewards = input.rewardEvents.map(event => ({
        kind: 'weapon_fragment',
    ...
    return {
        ...
        rewards: [
            ...rewards,
            ...(input.firstClear ? [{ kind: 'first_clear', rewardId: 'stage-first-clear', label: '章回首通', amount: 1, source: 'stage_first_clear', rarity: 'standard', firstClear: true }] : []),
        ],
        pity: null,
    };
}
function finalizePveSettlementDetail(detail, settlement) {
    const durableRewards = detail.rewards.filter(reward => reward.kind === 'weapon_fragment' || (reward.kind === 'first_clear' && reward.firstClear));
    if (settlement.goldGranted > 0)
        durableRewards.unshift({
            kind: 'gold', rewardId: `${settlement.settlementId}:gold`, label: '功勋金', amount: settlement.goldGranted,
            source: 'match_tier', rarity: 'standard',
        });
    if (settlement.entitlementIds.length > 0)
        durableRewards.push({
