...
export const BOSS_WAVE_NUMBERS = [5, 10, 15, 20] as const

export type BossWaveNumber = (typeof BOSS_WAVE_NUMBERS)[number]
export type BossRole = 'pressure' | 'support' | 'survival' | 'hybrid'
export type BossSkillPluginId = 'lane_minion_haste_v1' | 'phase_guard_v1'

export interface BossSkillBindingV1 {
  bindingId: string
  /** 战斗预警和状态栏只展示主题名，绝不能向玩家暴露 pluginId。 */
  displayName: string
  pluginId: BossSkillPluginId
  pluginVersion: 1
  trigger: 'periodic' | 'hp_threshold'
  /** 参数全部是整数；时间用毫秒、倍率用基点，运行时禁止解析描述文本。 */
  parameters: Readonly<Record<string, number>>
}

export interface BossDefinitionV1 {
  ...
  skills: readonly BossSkillBindingV1[]
}

export interface ResolvedBossStatsV1 {
  maxHp: number
  armor: number
  magicResistance: number
  moveSpeedMilliCellsPerSecond: number
  /** 10000 表示完全抵抗；首版永远小于 10000，不允许常驻免控。 */
  controlResistanceBps: number
  maxSingleControlDurationMs: number
  /** 插件数值的统一强度预算；插件逐字段夹断后应用。 */
  skillIntensityBps: number
}

export interface BossRewardProfileV1 {
  rewardProfileId: string
  rice: number
  /** 1000 = 1 点经验，与普通怪经验结算内部单位一致。 */
  experienceMilli: number
  /** 武器碎片仍由个人路线 Boss 节点完成事件统一结算。 */
  weaponMilestoneWave: BossWaveNumber
  directWeaponFragments: 0
}

export interface ResolvedBossEncounterV1 {
  schemaVersion: typeof BOSS_SCHEMA_VERSION
  catalogVersion: typeof BOSS_CATALOG_VERSION
  levelId: number
  difficulty: PveDifficulty
  waveNumber: BossWaveNumber
  definition: BossDefinitionV1
  stats: ResolvedBossStatsV1
  rewardProfile: BossRewardProfileV1
}

interface BossNodeBudgetV1 {
  waveNumber: BossWaveNumber
  hpRatioToOneOrdinaryBps: number
  defenseAdd: number
  moveSpeedMilliCellsPerSecond: number
  controlResistanceBps: number
  maxSingleControlDurationMs: number
...
const BOSS_NODE_BUDGETS: Readonly<Record<BossWaveNumber, BossNodeBudgetV1>> = Object.freeze({
  5: {
    waveNumber: 5,
...
const SKILL_DISPLAY_NAMES: Readonly<Record<string, string>> = Object.freeze({
...
function skillDisplayName(bindingId: string): string {
  const displayName = SKILL_DISPLAY_NAMES[bindingId]
  if (!displayName) throw new Error(`Missing boss skill display name: ${bindingId}`)
  return displayName
}
 ...
 */
export const BOSS_DEFINITIONS: readonly BossDefinitionV1[] = Object.freeze([
...
const BOSS_BY_STAGE_NODE: ReadonlyMap<string, BossDefinitionV1> = new Map(
  BOSS_DEFINITIONS.map((definition) => [`${definition.levelId}:${definition.waveNumber}`, definition]),
)

export function isBossWaveNumber(waveNumber: number): waveNumber is BossWaveNumber {
  return (BOSS_WAVE_NUMBERS as readonly number[]).includes(waveNumber)
}

export function getBossDefinition(levelId: number, waveNumber: number): BossDefinitionV1 | null {
  if (!isBossWaveNumber(waveNumber)) return null
  return BOSS_BY_STAGE_NODE.get(`${levelId}:${waveNumber}`) ?? null
}

function resolveSkillIntensityBps(levelId: number, difficulty: PveDifficulty): number {
  if (difficulty === 'easy') return 8500 + (levelId - 1) * 250
  if (difficulty === 'normal') return 11000 + (levelId - 1) * 300
  return 15500
}

export function resolveBossEncounter(
  levelId: number,
  difficulty: PveDifficulty,
  waveNumber: number,
): ResolvedBossEncounterV1 | null {
  if (!isBossWaveNumber(waveNumber)) return null
  // Boss 波即使目录缺失，也要先用统一入口拒绝非法关卡/难度，避免静默降级。
  resolvePveBalanceProfile(levelId, difficulty)
...
export function validateBossCatalog(): void {
  if (BOSS_DEFINITIONS.length !== 40) throw new Error('Boss catalog must define exactly 10 × 4 nodes')
  const ids = new Set<string>()
  const nodes = new Set<string>()
  for (const definition of BOSS_DEFINITIONS) {
    if (!Number.isInteger(definition.levelId)
      || definition.levelId < PVE_BALANCE_LEVEL_MIN
      || definition.levelId > PVE_BALANCE_LEVEL_MAX) {
      throw new Error(`Invalid boss levelId: ${definition.levelId}`)
    }
    if (!isBossWaveNumber(definition.waveNumber)) throw new Error(`Invalid boss wave: ${definition.waveNumber}`)
    if (Array.from(definition.glyph).length !== 1) throw new Error(`Boss glyph must be one Unicode glyph: ${definition.bossDefinitionId}`)
    if (ids.has(definition.bossDefinitionId)) throw new Error(`Duplicate bossDefinitionId: ${definition.bossDefinitionId}`)
    const nodeKey = `${definition.levelId}:${definition.waveNumber}`
    if (nodes.has(nodeKey)) throw new Error(`Duplicate boss node: ${nodeKey}`)
    if (definition.skills.length < 1) throw new Error(`Boss needs at least one registered skill: ${definition.bossDefinitionId}`)
    for (const skill of definition.skills) {
      if (skill.pluginVersion !== 1) throw new Error(`Unsupported boss plugin version: ${skill.pluginVersion}`)
      for (const value of Object.values(skill.parameters)) {
        if (!Number.isInteger(value) || value < 0) throw new Error(`Invalid boss skill parameter: ${skill.bindingId}`)
      }
    }
  ...
  for (let levelId = PVE_BALANCE_LEVEL_MIN; levelId <= PVE_BALANCE_LEVEL_MAX; levelId += 1) {
    for (const waveNumber of BOSS_WAVE_NUMBERS) {
      if (!nodes.has(`${levelId}:${waveNumber}`)) throw new Error(`Missing boss node: ${levelId}:${waveNumber}`)
    }
  }
}
