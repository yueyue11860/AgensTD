import { GENERAL_ROSTER, getGeneralRosterEntry } from '../core/hero-v1/roster'
import type { GeneralArchetype } from '../core/hero-v1/types'
import {
  WEAPON_FRAGMENT_REQUIREMENT,
  type WeaponDefinition,
  type WeaponEffectParameterPatch,
  type WeaponEventBudget,
  type WeaponQuality,
  type WeaponStatModifier,
  type WeaponTriggerDefinition,
} from './types'

const ZERO_BUDGET: WeaponEventBudget = {
...
const modifier = (
  effectId: string,
  target: WeaponStatModifier['target'],
  stat: WeaponStatModifier['stat'],
  operation: WeaponStatModifier['operation'],
  value: number,
  conditionTagsAny?: readonly string[],
): WeaponStatModifier => ({ effectId, target, stat, operation, value, conditionTagsAny })

const patch = (
  patchId: string,
  targetEffectId: string,
  parameter: string,
  operation: WeaponEffectParameterPatch['operation'],
  value: number,
  target: WeaponEffectParameterPatch['target'] = 'owner_general_effect',
): WeaponEffectParameterPatch => ({ patchId, target, targetEffectId, parameter, operation, value })

interface CommonWeaponInput {
  id: string
  name: string
  quality: WeaponQuality
  archetype: GeneralArchetype
  description: string
  stats?: readonly WeaponStatModifier[]
...
const common = (input: CommonWeaponInput): WeaponDefinition => ({
  ...
  fragmentRequirement: WEAPON_FRAGMENT_REQUIREMENT[input.quality],
...
export const WEAPON_CATALOG: readonly WeaponDefinition[] = [...COMMON_WEAPONS, ...EXCLUSIVE_WEAPONS]
export const WEAPON_CATALOG_BY_ID: ReadonlyMap<string, WeaponDefinition> = new Map(WEAPON_CATALOG.map((weapon) => [weapon.weaponId, weapon]))

const REGISTERED_TRIGGERS = new Set<string>([
...
export function validateWeaponCatalog(catalog: readonly WeaponDefinition[] = WEAPON_CATALOG): void {
  if (catalog.length !== 41) throw new Error(`Weapon catalog must contain 41 definitions, received ${catalog.length}`)
  const ids = new Set<string>()
  const names = new Set<string>()
  const exclusiveGenerals = new Set<string>()
  const qualityCounts: Record<WeaponQuality, number> = { green: 0, blue: 0, purple: 0, orange: 0, red: 0 }
  for (const weapon of catalog) {
    if (!/^[a-z][a-z0-9]*(?:_[a-z0-9]+)*$/.test(weapon.weaponId) || ids.has(weapon.weaponId)) throw new Error(`Invalid or duplicate weaponId: ${weapon.weaponId}`)
    if (!weapon.name || names.has(weapon.name)) throw new Error(`Missing or duplicate weapon name: ${weapon.name}`)
    ids.add(weapon.weaponId); names.add(weapon.name); qualityCounts[weapon.quality] += 1
    if (weapon.fragmentRequirement !== WEAPON_FRAGMENT_REQUIREMENT[weapon.quality]) throw new Error(`Fragment requirement mismatch: ${weapon.weaponId}`)
    if (!weapon.ui.shortDescription || !weapon.ui.detailDescription || !weapon.ui.iconKey) throw new Error(`Incomplete UI metadata: ${weapon.weaponId}`)
    for (const amount of Object.values(weapon.eventBudget)) if (!Number.isInteger(amount) || amount < 0) throw new Error(`Invalid event budget: ${weapon.weaponId}`)
    const exclusiveId = weapon.compatibility.exclusiveGeneralId
    if (exclusiveId) {
      if (weapon.quality !== 'red' || Object.keys(weapon.compatibility).length !== 1) throw new Error(`Exclusive weapon compatibility must contain only exclusiveGeneralId: ${weapon.weaponId}`)
      if (!getGeneralRosterEntry(exclusiveId) || exclusiveGenerals.has(exclusiveId)) throw new Error(`Invalid or duplicate exclusive general: ${exclusiveId}`)
    ...
    for (const effect of weapon.statModifiers) {
      if (effectIds.has(effect.effectId)) throw new Error(`Duplicate effectId in ${weapon.weaponId}: ${effect.effectId}`)
      effectIds.add(effect.effectId)
    }
    for (const trigger of weapon.triggers) {
      if (effectIds.has(trigger.triggerId)) throw new Error(`Duplicate projected effect ID in ${weapon.weaponId}: ${trigger.triggerId}`)
      effectIds.add(trigger.triggerId)
      if (!REGISTERED_TRIGGERS.has(trigger.kind) || !trigger.actions.length) throw new Error(`Invalid trigger in ${weapon.weaponId}: ${trigger.triggerId}`)
      if (trigger.kind === 'on_basic_attack_hit' && (trigger.chanceBps === undefined || trigger.perTargetIcdMs === undefined || trigger.maxTriggersPerSecond === undefined)) throw new Error(`Missing basic attack trigger guard: ${weapon.weaponId}`)
      if (trigger.kind === 'on_nth_basic_attack' && (!trigger.n || !trigger.counterScope)) throw new Error(`Missing nth attack guard: ${weapon.weaponId}`)
      if (trigger.kind === 'on_active_skill_cast' && trigger.maxTriggersPerCast === undefined) throw new Error(`Missing cast guard: ${weapon.weaponId}`)
      if (trigger.kind === 'on_active_skill_hit' && (trigger.perTargetIcdMs === undefined || trigger.maxTargetsPerCast === undefined)) throw new Error(`Missing skill hit guard: ${weapon.weaponId}`)
      if (trigger.kind === 'on_status_applied' && (!trigger.statusTags?.length || trigger.perTargetIcdMs === undefined)) throw new Error(`Missing status trigger guard: ${weapon.weaponId}`)
      if (trigger.kind === 'on_summon_basic_attack' && (!trigger.summonUnitFilter?.length || trigger.maxTriggersPerSecond === undefined)) throw new Error(`Missing summon attack guard: ${weapon.weaponId}`)
      if (trigger.kind === 'on_summoned_enemy_killed' && (!trigger.maxTriggersPerPeriod || !trigger.periodMs)) throw new Error(`Missing summon kill guard: ${weapon.weaponId}`)
      if (trigger.kind === 'on_enemy_killed' && !trigger.maxTriggersPerSkillCycle) throw new Error(`Missing enemy kill guard: ${weapon.weaponId}`)
    }
    for (const parameterPatch of weapon.parameterPatches) {
      if (effectIds.has(parameterPatch.patchId)) throw new Error(`Duplicate projected effect ID in ${weapon.weaponId}: ${parameterPatch.patchId}`)
      effectIds.add(parameterPatch.patchId)
      if (!parameterPatch.targetEffectId || !parameterPatch.parameter || !Number.isFinite(parameterPatch.value)) throw new Error(`Invalid parameter patch: ${weapon.weaponId}:${parameterPatch.patchId}`)
    }
    if (!effectIds.size) throw new Error(`Weapon has no executable projection: ${weapon.weaponId}`)
  }
  if (qualityCounts.green !== 4 || qualityCounts.blue !== 4 || qualityCounts.purple !== 4 || qualityCounts.orange !== 4 || qualityCounts.red !== 25) throw new Error(`Unexpected quality distribution: ${JSON.stringify(qualityCounts)}`)
  if (exclusiveGenerals.size !== 21 || GENERAL_ROSTER.some((general) => !exclusiveGenerals.has(general.generalId))) throw new Error('Every roster general must have exactly one exclusive weapon')
  for (const quality of ['green', 'blue', 'purple', 'orange', 'red'] as const) {
    ...
    if (commonAtQuality.length !== 4 || new Set(archetypes).size !== 4) throw new Error(`${quality} must contain one common weapon for each archetype`)
  }
}

export function getWeaponDefinition(weaponId: string): WeaponDefinition | null {
  return WEAPON_CATALOG_BY_ID.get(weaponId) ?? null
}

export function isWeaponCompatible(weapon: WeaponDefinition, generalId: string): boolean {
  const general = getGeneralRosterEntry(generalId)
  if (!general) return false
  const compatibility = weapon.compatibility
  if (compatibility.exclusiveGeneralId) return compatibility.exclusiveGeneralId === generalId
  if (compatibility.excludedGeneralIds?.includes(generalId)) return false
  if (compatibility.allowedGeneralIds?.includes(generalId)) return true
